...
**Combat tie** — when the winning owner's surviving ships equal zero after combat resolution (e.g., two equal-size fleets attack a zero-ship neutral simultaneously), the planet is set to `owner=-1` (neutral) with `ships=0`. The current owner does NOT win ties in this edge case — only when `surviving > 0` does ownership transfer. → [lookahead](src/lookahead.md)
