...
fails the suite. Run `python build.py` and commit the result **in the same change**
— it is a required artifact of that change, never an unrelated file and never
scope creep, even when an issue names only the `src/` file. Never hand-edit
`submission.py`; only regenerate it.
